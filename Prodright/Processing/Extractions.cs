...
{
    internal static class Extractions
    {

        internal static string GenerateXmlSummary(string sapPath)
        {
            try
            {
        ...
        internal static (List<SapConversion> conversions, List<StoreItem> storeItems) ParseXmlFiles1(string sapPath, string storePath)
        {
            ...
            // Generic EN description from SAP
            string sapGenericDesc = sapDoc.Descendants()
                .Where(e => e.Name.LocalName == "Description" && (string?)e.Attribute("languageCode") == "en")
                .Select(e => e.Value.Trim())
                .FirstOrDefault() ?? ""; // "CLOVER SUPER M 300ML STRAW" [2](https://sparza-my.sharepoint.com/personal/letourneurb_spar_net/Documents/Microsoft%20Copilot%20Chat%20Files/10283_sap.xml)

            // Per-UOM descriptions from SAP MeasureUnitCodeSpecificDescription
            ...
            var storeItems = storeDoc.Descendants(ns2 + "RetailSellingPrice")
                .Select(p =>
                {
                    ...
                    // Store-provided descriptions per price row [1](https://sparza-my.sharepoint.com/personal/letourneurb_spar_net/Documents/Microsoft%20Copilot%20Chat%20Files/10283_store.xml)
                    ...
                    // SAP fallback/enrichment
                    sapUomDesc.TryGetValue(uom ?? "", out var sapPerUom);

                    // If store values missing, fall back
                    itemDesc = string.IsNullOrWhiteSpace(itemDesc) ? sapGenericDesc : itemDesc;
                    scanDesc = string.IsNullOrWhiteSpace(scanDesc) ? sapPerUom ?? sapGenericDesc : scanDesc;
                    shelfDesc = string.IsNullOrWhiteSpace(shelfDesc) ? sapPerUom ?? sapGenericDesc : shelfDesc;
                    ...
                    return new StoreItem
                    {
                        ...
                        TypeCode = priceSpec?.Elements()?.FirstOrDefault(e => e.Name.LocalName == "PriceSpecificationElementTypeCode")?.Value,

                        // existing EAN logic
                        Eans = string.Join(", ", storeDoc.Descendants()
                            .Where(d => d.Name.LocalName == "Barcode")
                            .Where(b => b.Elements().FirstOrDefault(e => e.Name.LocalName == "MeasureUnitCode")?.Value?.Trim() == uom)
                            .Select(b => b.Elements().FirstOrDefault(e => e.Name.LocalName == "ProductStandardID")?.Value)),
        ...
        internal static (List<SapConversion> conversions, List<StoreItem> storeItems) ParseXmlFiles(string sapPath, string storePath)
        {
            XDocument sapDoc = XDocument.Load(sapPath);
            XDocument storeDoc = XDocument.Load(storePath);
        ...
        internal static DataTable CreateMappingTable(List<SapConversion> conversions, List<StoreItem> storeItems, Parameters p)
        {
